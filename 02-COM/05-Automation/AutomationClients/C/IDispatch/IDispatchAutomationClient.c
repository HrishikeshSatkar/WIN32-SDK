...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		VariantInit(&vRet);
		//sum
		hr = gpIDispatch->lpVtbl->GetIDsOfNames(gpIDispatch, &IID_NULL, &szFunctionOne, 1, GetUserDefaultLCID(), &dispid);
		...
		hr = gpIDispatch->lpVtbl->Invoke(gpIDispatch, dispid, &IID_NULL, GetUserDefaultLCID(), DISPATCH_METHOD, &params, &vRet, NULL, NULL);
		...
		//subtract
		hr = gpIDispatch->lpVtbl->GetIDsOfNames(gpIDispatch, &IID_NULL, &szFunctionTwo, 1, GetUserDefaultLCID(), &dispid);
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("GetIDsOfNames Failed!!!"), TEXT("Error"), MB_OK);
			DestroyWindow(hwnd);
		...
		if (gpIDispatch)
		{
			gpIDispatch->lpVtbl->Release(gpIDispatch);
			gpIDispatch = NULL;
		}
		DestroyWindow(hwnd);
		break;
