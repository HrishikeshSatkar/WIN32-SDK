...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("IMyMath Interface Can Not Be Obtained"), TEXT("Error"), MB_OK);
			DestroyWindow(hwnd);
		}
		...
		gpIMyMath->lpVtbl->SumOfTwoIntegers(gpIMyMath, iNum1, iNum2, &iSum);
		// display the result
		...
		gpIMyMath->lpVtbl->SubtractionOfTwoIntegers(gpIMyMath, iNum1, iNum2, &iSum);
		// display the result
		wsprintf(str, TEXT("Subtraction of %d and %d = %d"), iNum1, iNum2, iSum);
		MessageBox(hwnd, str, TEXT("Result"), MB_OK);
		// as ISubtract is now not needed onwards, release it
		gpIMyMath->lpVtbl->Release(gpIMyMath);
		gpIMyMath = NULL;// make the released interface null
